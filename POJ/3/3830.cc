//Result:wizmann	3830	Accepted	184K	16MS	C++	3218B	2012-10-15 10:37:11
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <iostream>
...
int main()
{
	...
	while(T--)
	{
		...
		for(int i=0;i<sz;i++)
		{
			...
			{
				maxlen=max(maxlen,pntDis(poly.array[i],poly.array[j]));
			}
		}
		scanf("%d",&csz);
		ConvexHull::init(csz);
		...
		for(int i=0;i<csz;i++)
		{
			...
			for(int k=0;k<csz;k++)
			{
				double dis=segment(ConvexHull::stack[i],ConvexHull::stack[j]).distoline(ConvexHull::stack[k]);
				tmp=max(tmp,dis);
			}
			coinlen=min(coinlen,tmp);
		}
